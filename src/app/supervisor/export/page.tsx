...
import { FileSpreadsheet, Download, RefreshCw, CheckCircle2, ArrowLeft, Calendar, DollarSign } from 'lucide-react'
    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <span className="text-xs font-bold text-emerald-400">Phase 3 • Supervisor Dashboard</span>
            <span className="text-slate-600">•</span>
            <span className="text-xs text-slate-400">Wage & CSV Export Center</span>
          </div>
          <h1 className="text-2xl font-extrabold text-white tracking-tight mt-1">
            Generate Official Wage Payout Spreadsheets
          </h1>
        </div>
        ...
        <button
          ...
          className="px-5 py-3 rounded-xl font-bold bg-emerald-500 text-slate-950 hover:bg-emerald-400 disabled:opacity-50 transition-all flex items-center gap-2 shadow-lg shadow-emerald-950/50 self-start sm:self-auto"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="glass-panel p-5 rounded-2xl border border-slate-800 space-y-2">
          <label className="block text-xs font-semibold text-slate-300">Select Site Work Session</label>
          <select
        ...
        <div className="glass-panel p-5 rounded-2xl border border-slate-800 space-y-1">
          <div className="text-xs text-slate-400 uppercase font-semibold">Total Verified Attendees</div>
          <div className="text-2xl font-extrabold text-white">{validRecords.length} Workers</div>
          <div className="text-xs text-emerald-400">Confirmed for daily wage payout</div>
        </div>

        <div className="glass-panel p-5 rounded-2xl border border-slate-800 space-y-1">
          <div className="text-xs text-slate-400 uppercase font-semibold">Total Computed Site Payout</div>
          <div className="text-2xl font-extrabold text-emerald-400">₹{totalPayout.toLocaleString('en-IN')}</div>
          <div className="text-xs text-slate-400">Calculated via daily wage rates</div>
        </div>
      </div>
      ...
      <div className="glass-panel p-6 rounded-3xl border border-slate-800 space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="font-bold text-white text-base">Session Wage Payout Preview</h2>
          <span className="text-xs text-slate-400">{validRecords.length} Eligible Records</span>
        </div>
              ...
              {validRecords.length === 0 ? (
                ...
                validRecords.map((r) => (
                  <tr key={r.id} className="hover:bg-slate-900/40">
                    <td className="py-3 px-4 font-semibold text-white">{r.worker?.name}</td>
                    <td className="py-3 px-4 font-mono text-xs text-slate-400">{r.worker?.phone}</td>
                    <td className="py-3 px-4">
                      <span className="px-2.5 py-0.5 rounded-full text-xs font-bold bg-emerald-500/15 text-emerald-400">
                        {r.status}
                      </span>
                    </td>
                    <td className="py-3 px-4 font-mono text-xs text-slate-300">1.0 Day</td>
                    <td className="py-3 px-4 font-mono text-xs text-slate-300">₹{r.worker?.wage_rate_per_day || 350}</td>
                    <td className="py-3 px-4 font-bold text-emerald-400">₹{r.worker?.wage_rate_per_day || 350}</td>
                  </tr>
                ))
              )}
            </tbody>
