  ...
  return (
    <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-slate-200/90 shadow-2xs">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
        {/* Brand Logo & Tagline */}
        <Link href={user ? (user.role === 'supervisor' ? '/supervisor/dashboard' : '/worker/dashboard') : '/'} className="flex items-center gap-2.5 sm:gap-3 group shrink-0">
          <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-slate-900 flex items-center justify-center border border-slate-800 shadow-sm group-hover:border-amber-500 transition-colors">
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          ...
          {/* Mobile Navigation Toggle (only when dashboard nav items exist) */}
          {navItems.length > 0 && (
            <button
